...
export function initLoadingShader(containerId) {
    ...
    const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true }); // alpha true for fade out
    ...
    function onPointer(event) {
        // Only trigger if clicking ON the canvas/loading screen
    ...
    document.addEventListener('click', onPointer); // document level to catch clicks anywhere while loading
    ...
    function animate() {
        if (!document.body.contains(container)) return; // Stop animation if container removed
